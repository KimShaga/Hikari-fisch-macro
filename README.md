...
사용 통계 전송은 기본적으로 꺼져 있습니다. 설정 탭의 **사용 통계 전송**을
켜면 설치 ID, 장치 식별값의 해시, 앱/OS 버전과 오프셋 상태 등이 OpenMacro로
전송됩니다. 통신은 비동기이며 끄면 진행 중인 요청도 취소합니다.

`pack-release.bat`는 사용자 설정·프리셋·로그를 삭제하지 않고 ZIP만 생성합니다.
...
- 헌트 감지
- Noiseform 매크로
- Lullaby 모드 자동 감지 및 낚시
- 자동 군주 인챈트 충전 (인챈트 게임패스 필요)
- 낚시 PID 개선 (이른 브레이킹 + 목표 통과 즉시 반전)
- 그 외 여러가지

앱 안 **크레딧 → XTernal과 다른 점**에서 더 최신 목록을 볼 수 있습니다.
