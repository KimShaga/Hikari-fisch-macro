...
ValidateAndSaveMain(key, ctrl, minValue, maxValue, isInteger := false, decimals := 2) {
    ...
    if (rawValue = "") {
        ...
        MsgBox("이 칸은 비울 수 없습니다.", "잘못된 값")
        return
    }

    if !RegExMatch(rawValue, "^-?(?:\d+|\d*\.\d+)$") {
        ctrl.Value := FormatSettingValue(oldValue, isInteger, decimals)
        MsgBox("유효한 숫자를 입력하세요.", "잘못된 값")
        return
    }
    ...
    if (numericValue < minValue || numericValue > maxValue) {
        ctrl.Value := FormatSettingValue(oldValue, isInteger, decimals)
        MsgBox("값은 " minValue " ~ " maxValue " 사이여야 합니다.", "범위 오류")
        return
    }
...
LoadConfig(name) {
    ...
    try {
        ...
        MsgBox("설정값 불러오기 실패: " err.Message, "설정값 오류")
    }
}
...
DeleteConfig(name) {
    ...
    try {
        ...
        MsgBox("설정값 삭제 실패: " err.Message, "설정값 오류")
    }
}
