; ============================================================================
;  Hikari's Edited Fisch Macro — fork changelog vs OpenMacro XTernal
; ============================================================================
#Requires AutoHotkey v2.0

GetHikariUpdateLog() {
    global FULL_VER
    return (
        FULL_VER " 핫픽스 (2026-09-13)`n"
        . "캐스팅 버그 수정: 최소 파워 2%, 타임아웃 5초 제한 제거`n"
        . "할리벗 하푼 버그 수정: 경고 검출 및 동시 포착 목표 계산 재구현`n"
        . "낚시 중앙 좌표·시간 기준 PWM 및 근거리 추적 개선`n"
        . "시작 시 GitHub 버전 확인 및 디스코드 가입 안내 추가`n"
        . "`n"
        . FULL_VER "`n"
        . "스텔라웨이브 매크로 기능 강화`n"
        . "낚시 도중 군주 충전 우선 버그 수정`n"
        . "재캐스팅 버그 수정`n"
        . "최적화 수정`n"
        . "`n"
        . "v0.7.3`n"
        . "스텔라웨이브 매크로 기능 추가`n"
        . "할리벗 하푼 매크로 기능 추가`n"
        . "최적화 수정`n"
        . "낚시 PID 벽 튕김 완화`n"
        . "`n"
        . "v0.7.2`n"
        . "최적화 문제 수정`n"
        . "감정 시스템 개편 및 보물섬 감정 추가`n"
        . "로블록스 연결 강화`n"
        . "`n"
        . "v0.7.1`n"
        . "창 사용 모드 추가`n"
        . "헌트 Apex 분류 추가 (Mosslurker, Beluga, Dreadfin, Magician Narwhal, Narwhal)`n"
        . "성큰 체스트 감지 추가`n"
        . "Humpback Whale이 감지되지 않던 문제 수정 및 위치 표시 기능 추가`n"
        . "낚시 PID 개선 (얇은 바 감속, 먼 거리 미끄러짐, 존 이동, 벽 튕김)`n"
        . "릴 디버그 오버레이/로그 추가`n"
        . "매크로 시작 시 이미 낚시 중이면 캐스트를 건너뛰고 잡기 단계로 진입`n"
        . "럴러바이 성능 개선 및 기능 개편`n"
        . "서버 첫 인챈트 Keepers 확인창 처리`n"
        . "`n"
        . "v0.6.2`n"
        . "GUI 환경 대거 개편`n"
        . "크레딧 수정`n"
        . "토템 지원 범위 확장`n"
        . "게임패스 감정 시스템 추가`n"
        . "인챈트 탭 / 게임패스 인챈트 추가`n"
        . "헌트 감지 시스템 추가`n"
        . "noiseform 매크로 기능 추가`n"
        . "Lullaby 모드 자동 감지 및 낚시 수행 기능 추가`n"
        . "자동 군주 인챈트 충전(인챈트 게임패스 필요) 추가`n"
        . "낚시 PID 성능 개선 (이른 브레이킹 + 목표 통과 즉시 반전)`n"
    )
}
