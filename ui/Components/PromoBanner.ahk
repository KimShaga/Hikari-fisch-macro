...
class PromoBanner {
    ...
    static ENABLED  := false                       ; master switch for the strip
    ...
    static Attach(gui, accentHex, bgHex, textHex, borderHex) {
        ...
        this._rule := Border(gui, 0, 0, 880, 3, accentHex)         ; accent rule at the very top

        gui.SetFont("s16 bold", "Segoe UI Emoji")
        this._icon := gui.AddText("x5 y10 w30 h30 c" textHex, "🎣")

        gui.SetFont("s10 bold", "Segoe UI")
        this._head := gui.AddText("x40 y10 w520 h18 c" textHex,
            "Try " this.PRODUCT " free for " this.DAYS " days")

        gui.SetFont("s8 norm", "Segoe UI")
        this._sub := gui.AddText("x40 y30 w520 h14 c" sub,
            "Full macro · no card · claim in Discord")

        ; Right column: a bold accent badge over the CTA link.
        gui.SetFont("s11 bold", "Segoe UI")
        this._badge := gui.AddText("x700 y8 w160 h20 Right c" accentHex, this.DAYS "-DAY FREE")

        gui.SetFont("s8 norm underline", "Segoe UI")
        this._cta := gui.AddText("x700 y31 w160 h12 Right c" accentHex, "Start free trial →")

        Border(gui, 0, H - 1, 880, 1, borderHex)                   ; divider under the strip
